...
class AdminCaseController extends Controller
{
    ...
    public function update(
        ...
        if ($admin->canManageStaff()) {
            $rules['assigned_admin_user_id'] = ['nullable', 'integer', 'exists:admin_users,id'];
        }
        $validated = $request->validate($rules);

        $case = $this->visibleCases($admin)->where('public_id', strtoupper($publicId))->firstOrFail();
        ...
        if ($admin->canManageStaff()
            && array_key_exists('assigned_admin_user_id', $validated)
